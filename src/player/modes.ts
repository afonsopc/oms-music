...
export const stemNodeIdForMode = (song: Song, mode: PlaybackMode): FsNodeId | null => {
  if (mode === "instrumental") return song.instrumental_media_id ?? null;
  if (mode === "vocals") return song.vocals_media_id ?? null;
  return null;
};
...
export const stemPairNodeIds = (
  ...
  const vocals = song.vocals_media_id ?? null;
  const instrumental = song.instrumental_media_id ?? null;
...
export const localKindsForMode = (song: Song, mode: PlaybackMode): DownloadKind[] => {
  if (mode === "instrumental" && song.instrumental_media_id) return ["instrumental"];
  if (mode === "vocals" && song.vocals_media_id) return ["vocal"];
