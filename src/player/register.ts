...
const engineTransport = (e: PlayerEngineImpl): TransportActions => ({
  ...
  setPlaybackMode: (mode) => e.setPlaybackMode(mode),
  setEqBand: (band, db) => e.setEqBand(band, db),
  setEqEnabled: (on) => e.setEqEnabled(on),
  setVocalVolume: (volume) => e.setVocalVolume(volume),
  setInstrumentalVolume: (volume) => e.setInstrumentalVolume(volume),
