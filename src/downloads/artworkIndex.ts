 ...
 * bare fs node (`deriveOfflineAlbums` carries an `artwork_fs_node_id` and
...
type ArtworkCarrier = Pick<Song, "artwork_fs_node_id" | "compressed_artwork_fs_node_id">;

/** Every node id under which this song's artwork can be requested. */
export const artworkNodeIdsOf = (song: ArtworkCarrier): FsNodeId[] => {
  const ids: FsNodeId[] = [];
  for (const id of [song.compressed_artwork_fs_node_id, song.artwork_fs_node_id]) {
