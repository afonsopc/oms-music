...
- **Typography:** Inter for body; Druk Wide Super (weight 900) and Cantarell as display faces. Hero titles are huge black-weight; section headers `text-2xl` bold tight; kind labels tiny uppercase; time labels tabular numerals. Mix "stamp" text is black-weight uppercase white, size stepped down by text length (<=8 chars largest, <=14, <=22, else smallest).
